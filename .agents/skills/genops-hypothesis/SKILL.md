...
**Config:** `id: hypothesis`, `requires: [docs/research/]`, `outputs: docs/hypothesis/`, `file_pattern: HYPOTH-{NNN}-{slug}.md`, `template: research/HYPOTHESIS.md.template`, `next: [experiment]`

## Execution Protocol

1. **PRE-FLIGHT** — Verify `docs/research/` exists with approved review. Verify `research/HYPOTHESIS.md.template` exists.
2. **LOAD** — Read upstream literature review and `.agents/context/CONTEXT.md`.
3. **DOMAINS** — Discover domains from upstream review.
4. **CHECK** — Compute LF-normalized hash of `docs/research/`.
